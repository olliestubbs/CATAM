...
def findBases(U,V,mod):
    Urowspace = gaussElim(U,mod)
    Vrowspace = gaussElim(V,mod)
    fullspace = np.vstack((Urowspace,Vrowspace))
    UplusV = gaussElim(fullspace,mod)
    U0 = np.array(findKernel(U,mod),dtype = 'int64')
    V0 = np.array(findKernel(V,mod),dtype = 'int64')
    
    U0plusV0 = np.vstack((U0,V0))
    U0plusV0 = gaussElim(U0plusV0,mod)
    UintV=np.array(findKernel(U0plusV0,mod),dtype = 'int64')
    return [Urowspace,Vrowspace,UplusV,UintV]
...
A3019 = np.array(findKernel(A3,7),dtype = 'int64')
